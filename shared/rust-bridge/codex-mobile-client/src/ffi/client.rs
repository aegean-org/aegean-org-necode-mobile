...
impl AppClient {
    ...
    /// platform should now persist. Idempotent: subsequent calls
    /// return the same bytes from the already-bound endpoint.
    ///
    ...
    pub async fn ensure_alleycat_endpoint(&self) -> Result<Vec<u8>, String> {
        let endpoint = self
            .inner
            .alleycat_endpoint()
            .await
            .map_err(|error| error.to_string())?;
        Ok(endpoint.secret_key().to_bytes().to_vec())
    }
